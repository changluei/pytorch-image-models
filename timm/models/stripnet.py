...
try:
    # Keep compatibility when this file is reused in mmrotate-style codebases.
    from ..builder import ROTATED_BACKBONES
except Exception:
    ROTATED_BACKBONES = None


def _maybe_register_rotated_backbone(cls):
    if ROTATED_BACKBONES is not None:
        ROTATED_BACKBONES.register_module()(cls)
    return cls
...
@register_model
@_maybe_register_rotated_backbone
class StripNet(BaseModule):
    def __init__(self, img_size=224, in_chans=3, embed_dims=[64, 128, 256, 512],
        ...
        assert not (init_cfg and isinstance(pretrained, str)), \
        ...
        if isinstance(pretrained, str):
            ...
            if pretrained:
                warnings.warn(
                    '`pretrained=True` is set, but no timm pretrained weights are registered for StripNet. '
...
@register_model
def stripnet(pretrained=False, **kwargs):
    return StripNet(pretrained=pretrained, **kwargs)
